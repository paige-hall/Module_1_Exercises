...
public class Exercises {
    ...
    public double isItOnSale(String itemNumber) {
        Map<String, Double> onSale = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
        ...
        if (itemNumber == null || itemNumber == "" || (!onSale.containsKey(itemNumber))) {
            return 0.00;
        }
    ...
    public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
        ...
        if (petersMoney >= 5000 && paulsMoney >= 10000) {
            int peterOneFourth = petersMoney / 4;
            petersMoney = petersMoney - peterOneFourth;
            int paulOneFourth = paulsMoney / 4;
            paulsMoney = paulsMoney - paulOneFourth;

            int peterAndPaul = peterOneFourth + paulOneFourth;
            peterPaul.put("Peter", petersMoney);
            peterPaul.put("Paul", paulsMoney);
            peterPaul.put("PeterPaulPartnership", peterAndPaul);
            return peterPaul;
        }
        return peterPaul;
    }
    ...
    public Map<String, String> beginningAndEnding(String[] words) {
        ...
        for (String word : words) {
            firstAndLast.put(word.substring(0, 1), word.substring(word.length() - 1));
        }
    ...
    public Map<Integer, Integer> integerCount(int[] ints) {
        ...
        for (Integer intList : ints) {
            if (count.containsKey(intList)) {
                int currentCount = count.get(intList);
                currentCount++;
                count.put(intList, currentCount);
            } else {
                count.put(intList, 1);
            }
        }
    ...
    public Map<String, Boolean> wordMultiple(String[] words) {
        ...
        Map<String, Integer> twoTrue = new HashMap<>();

        for (String repeat : words) {

            if (twoTrue.containsKey(repeat)) {
                int currentCount = twoTrue.get(repeat);
                currentCount++;
                if (currentCount > 1) {
                    multiples.put(repeat, true);
                } else {
                    multiples.put(repeat, false);
                }
            } else {
                twoTrue.put(repeat, 1);
                multiples.put(repeat, false);
            }
        }
        return multiples;
    ...
    public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse, Map<String, Integer> remoteWarehouse) {
        ...
        for (String name : mainWarehouse.keySet()) {
            if (remoteWarehouse.containsKey(name)) {
                comboMap.put(name, (mainWarehouse.get(name) + remoteWarehouse.get(name)));
            } else {
                comboMap.put(name, mainWarehouse.get(name));
            }
        }
        for (String name : remoteWarehouse.keySet()) {
            if (!(mainWarehouse.containsKey(name))) {
                comboMap.put(name, remoteWarehouse.get(name));
            }
        }
        return comboMap;
    ...
    public Map<String, Integer> last2Revisited(String[] words) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            int counterA = 0;
            for (int counterB = 0; counterB < temp.length() - 1; counterB++) {
                if (temp.substring(counterB).startsWith(lastTwo))
                    counterA++;
            }
            newMap.put(temp, counterA - 1);
        }
        return newMap;
